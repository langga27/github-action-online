...
detect_ip_configuration(resource) {
...
deny[msg] {
  ...
  msg := {
    # Mandatory fields
    "publicId": "GCPPQL07",
    "title": "GCPPQL07: Network Access",
    "severity": "high",
    "msg": sprintf("resource.google_sql_database_instance[%s]", [name]),
    # Optional fields
    "issue": "Access to Group GCP Cloud SQL for PostgreSQL Database must be restricted to Internal IP only via private services access, public IP must be disabled and no authorized networks is allowed.",
