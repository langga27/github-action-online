...
##### SOUR
#This one for range
check_port_remote_shell(resource) { 
	banPorts := [513]
...
check_port_remote_shell(resource) { 
	resource.source_port_range == "513"
}

check_port_remote_shell(resource) { 
	resource.source_port_range == "514"
}
...
#This one for individual string type
check_port_remote_shell(resource) { 
	resource.destination_port_range == "513"
}

check_port_remote_shell(resource) { 
	resource.destination_port_range == "514"
}
#this one for individual num type
check_port_remote_shell(resource) { 
	resource.destination_port_range == 513
...
deny[msg] {
	...
	msg := {
		# Mandatory fields
		"publicId": "AZURE-NSG-NETWORK-HYGIENE-REMOTE-SHELL", #please change the public id
		"title": "Network Hygiene REMOTE-SHELL (tcp 513/514)",
		"severity": "high",
	    "msg": sprintf("resource.azurerm_network_security_rule[%s]", [name]),
		"issue": "REMOTE-SHELL ports (513/514) via TCP shall not be used, unless it is within Prudential internal network (within 10.0.0.0/8)",
		"impact": "REMOTE-SHELL default port should not be accessible from un-trusted network for administrative purpose.",
		"remediation": "Please remove the use of REMOTE-SHELL ports (513/514). You may refer to the baseline documents in https://pruo365.sharepoint.com/:b:/r/sites/GROUP-1-HUB/Shared%20Documents/Group%20Governance%20Manual/5.%20GGM%20Policies/Group%20Technology/Group%20Technology/Information%20Security/Group%20Banned%20Protocol%20Standard%20%5B2023%5D.pdf?csf=1&web=1&e=kjAZIK for more details.",
		"references": [""],
	}
}