...
#This one for individual string type
check_port_23(resource) { 
	resource.destination_port_range == "23"
}
#this one for individual num type
check_port_23(resource) { 
	resource.destination_port_range == 23
...
deny[msg] {
	...
	msg := {
		# Mandatory fields
		"publicId": "AZURE-NSG-NETWORK-HYGIENE-TELNET", #please change the public id
		"title": "Network Hygiene TELNET (tcp 23)",
		"severity": "high",
	    "msg": sprintf("resource.azurerm_network_security_rule[%s]", [name]),
		"issue": "TELNET ports (23) via TCP shall not be used, unless it is within Prudential internal network (within 10.0.0.0/8)",
		"impact": "TELNET default port should not be accessible from un-trusted network for administrative purpose.",
		"remediation": "Please remove the use of TELNET ports (23). You may refer to the baseline documents in https://pruo365.sharepoint.com/:b:/r/sites/GROUP-1-HUB/Shared%20Documents/Group%20Governance%20Manual/5.%20GGM%20Policies/Group%20Technology/Group%20Technology/Information%20Security/Group%20Banned%20Protocol%20Standard%20%5B2023%5D.pdf?csf=1&web=1&e=kjAZIK for more details.",
		"references": [""],
	}
}