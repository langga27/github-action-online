...
check_gcp_banned_protocol_ibm_http_server(resource) {
...
deny[msg] {
    ...
    msg := {
        # Mandatory fields
        "name": [name],
        "publicId": "GCP-BANNED-PROTOCOL-IBM-HTTP-SERVER",
        "title": "Banned Protocols IBM-HTTP-SERVER (tcp 8008)",
        "severity": "high",
        "msg": sprintf("resource.google_compute_firewall[%s]", [name]),
        "issue": "IBM Http Server ports (8008) via TCP shall not be used as these are insecure protocol for common web service admin, unless it is from a trusted network.",
        "impact": "For web-based administration of a HIS server. Administrations cannot be performed from external (Internet)",
        "remediation": "Please remove the use of IBM Http Server port (8008). You may refer to the baseline documents in https://pruo365.sharepoint.com/:b:/r/sites/GROUP-1-HUB/Shared%20Documents/Group%20Governance%20Manual/5.%20GGM%20Policies/Group%20Technology/Group%20Technology/Information%20Security/Group%20Banned%20Protocol%20Standard%20%5B2023%5D.pdf?csf=1&web=1&e=kjAZIK for more details.",
        "references": [""],
    }
}