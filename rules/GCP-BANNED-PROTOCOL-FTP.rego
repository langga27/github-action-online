...
check_gcp_banned_protocol_ftp(resource) {
...
deny[msg] {
    ...
    msg := {
        # Mandatory fields
        "name": [name],
        "publicId": "GCP-BANNED-PROTOCOL-FTP",
        "title": "Banned Protocols FTP (tcp 20, 21)",
        "severity": "high",
        "msg": sprintf("resource.google_compute_firewall[%s]", [name]),
        "issue": "FTP ports (20 and 21) via TCP shall not be used, unless it is within Prudential internal network (within 10.0.0.0/8)",
        "impact": "Insecure file transfer where both data and credentials are transmitted in clear which can be intercepted during transmission. This could lead to malicious user obtaining the data or credentials.",
