...
deny[msg] {
	...
	msg := {
		# Mandatory fields
		"publicId": "AZURE-NSG-NETWORK-HYGIENE-SMB", #please change the public id
		"title": "Network Hygiene SMB (tcp 445)",
		"severity": "high",
	    "msg": sprintf("resource.azurerm_network_security_rule[%s]", [name]),
		"issue": "SMB ports (445) via TCP shall not be used, unless it is within Prudential internal network (within 10.0.0.0/8)",
		"impact": "SMB default port should not be accessible from un-trusted network for administrative purpose.",
